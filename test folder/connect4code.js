...
function generateHTMLBoard(NBRANGEES, NBCOLONNES){
	...
	for(var i = (NBRANGEES-1) ; i >= 0 ; i--){
		...
		for(var j = 0; j < NBCOLONNES; j++){
			//var boxText ='<li class="playbox" id="+i+j"></li>';
			//The row element
...
function colonneAcoord(board, play){          	// Working
...
function clearHTMLBoard(){
	$("li").css("background-image","url(images/blankquare.png)");
	$("li").removeClass("highlightbox").addClass("playbox");
}
...
while (vainqueur < 1){                              // Boucle jusqu'a ce qu'un joueur gagne, ou qu'une partie es nulle.
	...
	// Losrsque sorti de la boucle, il y a forcément un gagnant. On déclare (k % 2) comme gagnant
    if((k % 2) == 0){
        alert("\nJoueur 1 gagnant!");
        return 0;
        }
       ...
       alert("\nJoueur 2 gagnant!");
		...
		$(".playbox").mouseenter();	// Show a token of the player's color
		$(".playbox").mouseleave();	// Return to the previous image
		$(".playbox").click();		//  
};

$("#startgame").click(function(){ 	//Run game when user clicks start game
	clearHTMLBoard()				//Clear all the squares from previous game
	connect4();						//Start a new game
})
